...
plink --noweb --bfile $tmp1 --extract $out.prune.in --mind 0.1 --make-bed --out $tmp2.pruned

# step 6: cryptic relatedness/IBD check
# IBD check (make .genome file)
plink --noweb --bfile $tmp2.pruned --genome --min 0.05 --make-bed --out $tmp1.pruned
# remove relateds
plink --noweb --bfile $tmp1.pruned --rel-cutoff --make-bed --out $tmp2.pruned
...
plink --noweb --bfile $tmp2.pruned --list-duplicate-vars ids-only suppress-first --out $tmp2.pruned

plink --noweb --bfile $tmp2.pruned --remove $tmp2.pruned.dupvar --make-bed --out $tmp1.pruned

# recode
plink --noweb --bfile $tmp1.pruned --recode --out $tmp1.pruned

smartpca.perl -i $tmp1.pruned.ped -a $tmp1.pruned.map -b $tmp1.pruned.fam -s 6 \
